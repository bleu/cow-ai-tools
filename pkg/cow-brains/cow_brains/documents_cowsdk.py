"""CoW SDK (cowprotocol/cow-sdk) README + docs for RAG. Same artifact format as cow_docs (==> path <==)."""
import re
import time
...
from cow_brains.config import COW_SDK_DOCS_PATH

NOW = time.strftime("%Y-%m-%dT%H:%M:%S.000Z", time.gmtime())
GITHUB_BASE = "https://github.com/cowprotocol/cow-sdk/blob/main/"


def _path_to_url(rel_path: str) -> str:
    """Build GitHub blob URL for cow-sdk file."""
    s = rel_path.strip()
    return GITHUB_BASE + s


class CowSdkDocsProcessingStrategy:
    name_source = "cow_sdk_docs"
    ...
    async def langchain_process(
        ...
        **kwargs,
        ...
        path = file_path or COW_SDK_DOCS_PATH or ""
        if not path:
            return []
        try:
    ...
    async def dataframe_process(**kwargs) -> pd.DataFrame:
        fragments = await CowSdkDocsProcessingStrategy.langchain_process(**kwargs)
        if not fragments:
